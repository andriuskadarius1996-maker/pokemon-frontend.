...
export default function App(){
  return (
    <div style={{padding:24, maxWidth:1200, margin:'0 auto'}}>
      <div style={{background:'#0f1a33', padding:20, borderRadius:16, boxShadow:'0 10px 30px rgba(0,0,0,.4)'}}>
        <h1 style={{margin:0}}>POKE TOKEN — Season 1</h1>
        <p style={{opacity:.8}}>Avatarų patikra (kraunama iš <code>public/avatars_main/</code> per manifestą).</p>
      </div>
      <div style={{display:'grid', gridTemplateColumns:'repeat(auto-fill,minmax(140px,1fr))', gap:16, marginTop:24}}>
        {levels.map(n=>{
          const key = String(n)
          const url = (manifest as any)[key] as string
          return (
