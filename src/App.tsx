...
function Tabs({tab, setTab}:{tab:Tab; setTab:(t:Tab)=>void}){
  ...
  return (
    <div className="flex gap-2 flex-wrap">
      {names.map(n=> (
        <button key={n} className={`tab ${tab===n? "tab-active":""}`} onClick={()=>setTab(n)}>{n}</button>
      ))}
    </div>
  );
}
...
function Header(){
  ...
  useEffect(()=>{
    const id = setInterval(()=>force(), 1000); // update timers
  ...
  return (
    <div className="card mt-4">
      ...
      <div className="grid grid-cols-6 gap-4 mt-6">
        {avatars.map((src,i)=>(
          <AvatarAnimated key={i} src={src} label={`lv${String(i+1).padStart(2,"0")}`} />
        ))}
      </div>
    </div>
  );
...
function StakeTab(){
  const names = ["Sparko","Embero","Aquado","Legendary","Pikachu"];
  return (
    <div className="grid md:grid-cols-2 gap-4 mt-4">
      {names.map(n=> <div key={n}><(require("./components/StakeCard")).default name={n}/></div>)}
    </div>
  );
}
...
function ShopTab(){
  return (
    <div className="grid md:grid-cols-3 gap-4 mt-4">
      <div className="card">
        <div className="title">XP Boost x2 (9h)</div>
        <p className="opacity-80 mt-2">Price: 200 PT</p>
        <button className="mt-4 px-4 py-2 rounded-xl bg-white text-black" onClick={()=>actions.buy("xp")}>Buy</button>
      </div>
      <div className="card">
        <div className="title">Stake Boost +2% (9h)</div>
        <p className="opacity-80 mt-2">Price: 250 PT</p>
        <button className="mt-4 px-4 py-2 rounded-xl bg-white text-black" onClick={()=>actions.buy("stake")}>Buy</button>
      </div>
      <div className="card">
        <div className="title">Small Chest</div>
        <p className="opacity-80 mt-2">Drops: PT/XP</p>
        <button className="mt-4 px-4 py-2 rounded-xl bg-white text-black" onClick={()=>{ actions.addPT(50); actions.addXP(20); }}>Open</button>
      </div>
    </div>
  );
}
...
function LeaderboardTab(){
  const rows = Array.from({length:10},(_,i)=>({
    ...
    xp:(10-i)*1000,
    pt:(10-i)*200,
    prize: i<3? ["13 SOL","8 SOL","6 SOL"][i]:"0.95 SOL"
  }));
  return (
    <div className="card mt-4">
      ...
      <div className="overflow-x-auto mt-4">
        <table className="min-w-full text-sm">
          <thead className="opacity-70">
            <tr><th className="text-left p-2">#</th><th className="text-left p-2">Nick</th><th className="text-left p-2">XP</th><th className="text-left p-2">PT</th><th className="text-left p-2">Prize</th></tr>
          </thead>
          <tbody>
            {rows.map((r,i)=>(
